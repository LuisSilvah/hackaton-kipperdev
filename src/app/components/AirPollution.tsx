...
export function AirPollution() {
  ...
  const [airQualityPollution, setAirQualityPollution] =
    ...
    const [weather, setWeather] =
      ...
      const [airPollutionColor, setAirPollutionColor] = useState<AirPollutionColorType>("green")

  const api = {
    apiKey: "f1edf5cc13a848146c771f243dbf8c58",
  };
  ...
  const fetchAirQualiPollution = async (lat: number, lon: number) => {
    try {
      const response = await fetch(
        `http://api.openweathermap.org/data/2.5/air_pollution?lat=${lat}&lon=${lon}&appid=${api.apiKey}`
      );
...
//               | Pode causar sérios problemas de saúde para todos, e a saúde das pessoas pode ser afetada imediatamente.
  ...
  return (
    <div className="flex flex-col items-start gap-4 mobile:items-center">
      ...
      <span className="text-xl font-roboto font-medium">{weather?.main.humidity}°C</span>
      </div>
      <div className="flex gap-4">
      <FilterIcon className="w-8 h-8" color={airPollutionColor} />
      <span className="text-xl font-roboto font-medium">Nível de Poluição: {airQualityPollution?.list[0].main.aqi}</span>
      </div>
    </div>
  );
}
